...
// Add at the bottom of lib.rs before or after crawl_github_files function
...
#[pymodule]
fn github_crawler(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(crawl_github_files_py, m)?)?;
    Ok(())
}